...
struct Change
{
	...
	Change(Address const& _addr, Address const& _pk, Address const& contract_addr, ContractFun const& contrac_fun, Weight weight) : kind(ControlContract), address(_addr){
		...
		control_contract.second.second.second = weight;
	}
};
...
public:
	...
	void execute_account_control_contract_fun(Address const& _from, std::vector<std::shared_ptr<transationTool::operation>> const& _ops);
